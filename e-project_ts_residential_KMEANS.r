...
# IN THIS VERSION FOCUSING ON RESIDENTAIL CUSTOMERS THAT HAVE HAND-PICKED BY THEIR PATTERN
...
# Filter out specific cube_ids
excluded_ids <- c(920, 199, 1285, 1495, 1632, 1488, 1597, 1635, 1724, 51, 331, 538, 860, 973, 75, 1639, 1727) # those are the COMMERCIAL CUSTOMERS
combined_data <- combined_data %>% filter(!(cube_id %in% excluded_ids))
...
plot_specific_cluster <- function(data, cluster_number, start_date, end_date, line_color = "green") {
  data_to_plot <- data %>%
...
# Call the function to plot cluster X from June 1, 2023, to July 1, 2023, in clor you want
plot_specific_cluster(combined_data, 3, "2023-10-29", "2023-10-30", "green")
...
highest_peaks_cluster <- get_highest_peaks_by_cluster(combined_data, 5)
### Print the result
...
save_to_csv <- function(data, filename = "tbl_kmeans_22-23_expeaks_1h.csv") {
  data_to_save <- data %>% 
